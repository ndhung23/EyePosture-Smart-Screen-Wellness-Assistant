...
| **Client Secrets Leakage** | Critical | Strict rule: Stripe secret keys, JWT signing keys, and cloud database credentials are never bundled in desktop builds. Only public verification keys exist on the client. |
...
| **Fake Webhook Requests** | High | Webhook signatures verified from Stripe headers. Event IDs tracked in idempotency set to reject replay attacks. |
...
- **Backend API Server**: Loads `JWT_SECRET`, `ENTITLEMENT_SECRET`, and `STRIPE_SECRET_KEY` strictly through environment variables.
